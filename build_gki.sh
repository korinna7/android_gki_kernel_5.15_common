...
show_usage() {
    ...
    echo "  --susfs        Enable SUSFS support for KernelSU (implies --ksu)"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --susfs)
            ENABLE_SUSFS=true
            ENABLE_KSU=true      # SUSFS requires KernelSU
            shift
            ;;
        --ak3)
            PACK_AK3=true
...
# 3.2 Setup SUSFS (if enabled, requires KSU to be set up first)
